...
const buildSenotype = (assertions) => {
    ...
    let microenvironmentChildren = buildAssertionChildren(assertions, "has_microenvironment")
            ...
            children: <span className={'flex'}>
                        {celltypeChildren.map((item, index) => (
                            <div key={`celltype_${index}`} className={'mb-1'}>
                                {item.key} ({item.value}) <a target={'_blank'}
                                                             href={getOboDetailsUrl(item.key.replace(':', '_'))}>
                                <LinkOutlined/></a>
                            </div>
                        ))}
    ...
    const markerColumns = (title, dataIndex) => [
        {
            ...
            render: (_, record) =>
                <span>{record[dataIndex]} <a target={'_blank'}
                    ...
                    <div className={'markdown'}>
                        ...
                        <AppAccordion title={'Specified Markers'} id={'specified-markers'}>
                            <Table
                                ...
                                dataSource={buildMarkers(senotype.assertions, 'specified_marker', 'has_characterizing_marker_set')}></Table>
                        </AppAccordion>
                        ...
                        <AppAccordion title={'Regulating Markers'} id={'regulating-markers'}>
                            <Table columns={[
                                ...markerColumns('Regulating Marker', 'regulating_marker'),
                                {
                                   ...
                                   dataSource={[
                                       ...buildMarkers(senotype.assertions, 'regulating_marker', 'down_regulates', 'down'),
                                       ...buildMarkers(senotype.assertions, 'regulating_marker', 'up_regulates', 'up'),
                                       ...buildMarkers(senotype.assertions, 'regulating_marker', 'inconclusively_regulates', '?'),
                                   ]}
